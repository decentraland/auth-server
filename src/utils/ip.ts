...
export async function createIpUtilsComponent({ logs }: Pick<AppComponents, 'logs'>): Promise<IIpUtilsComponent> {
  ...
  const getClientIpWithLogging = (input: GetClientIpInput): string => {
  ...
  return {
    normalizeIp,
    getClientIp: getClientIpWithLogging,
    ipsMatch,
    formatIpHeaders
  }
}
...
export function getClientIp(input: GetClientIpInput): string {
...
export function formatIpHeaders(headers: IpHeaders): string {
